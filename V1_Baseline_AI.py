...
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0' # Turn off specialized operations
os.environ['CUDA_VISIBLE_DEVICES'] = '0' # Ensure only the GPU is used
...
OUTPUT_FOLDER = "../walk_forward_results"
...
if not os.path.exists(MODELS_FOLDER): os.makedirs(MODELS_FOLDER) # NEW
...
def build_quant_model(input_shape):
    model = Sequential([
        ...
        # Reset the activation and recurrent_activation 
        # to disable the buggy CUDNN implementation.
...
def main():
    ...
    for fold_config in FOLDS:
        ...
        print(f"\n{'='*50}")
        print(f"RUNNING FOLD {fold_num}/12")
        print(f"{'='*50}")
        ...
        model_save_path = os.path.join(MODELS_FOLDER, f"Quant_Model_Fold_{fold_num}.keras")
        
        callbacks = [
            # Stops if validation loss doesn't improve for 5 epochs
            EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True, verbose=1),
            # Drops learning rate if AI gets stuck
            ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=2, min_lr=0.00001, verbose=1),
            # Saves the physical model file to your hard drive!
