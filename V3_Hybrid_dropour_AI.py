...
OUTPUT_FOLDER = "../walk_forward_results_V3"
MODELS_FOLDER = "../saved_models_V3"
...
# We ONLY drop the most toxic, absolute-value noise.
TOXIC_FEATURES = [
    'Close', 'High', 'Low', 'SMA_20', 'SP500_200MA'
]
...
def process_stock_for_fold(df, ticker, train_end, test_start, test_end):
    # DROP ONLY THE ABSOLUTE WORST NOISE
...
def main():
    ...
    if not feature_files:
        print("CRITICAL ERROR: No feature files found.")
        return
    ...
    for fold_config in FOLDS:
        ...
        print(f"\n{'='*50}")
        print(f"RUNNING FOLD {fold_num}/12 (V3 SOFT PRUNED & 40% DROPOUT)")
        print(f"{'='*50}")
        ...
        model_save_path = os.path.join(MODELS_FOLDER, f"Quant_Model_V3_Fold_{fold_num}.keras")
